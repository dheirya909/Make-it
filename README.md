...
* 🎛️ **Procedural UI Audio (Web Audio API):** Instead of using static `.mp3` files for button clicks, the UI generates a synthesized "metal click" with algorithmic convolution reverb in real-time, preventing audio stacking during rapid clicks.
* 🗺️ **Live Interactive SVG Map:** A dynamic nodal transit map that tracks "live traffic" (high/medium/low) and prevents users from booking pods into congested zones without a warning.
